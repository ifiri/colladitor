...
export function buildKeymap(schema: Schema, mapKeys?: { [key: string]: false | string }) {
  ...
  if ((type = schema.nodes.subscription_form)) {
    bind('Backspace', function (state, dispatch, view) {
      const selectParent = (selection) => {
        ...
        if (same !== 0) {
          ...
          return $from.doc.resolve(pos);
        }

        return false;
      };

      const pos = selectParent(state.selection);

      if (pos && pos.parent?.type.name === 'subscription_form') {
  ...
  if ((type = schema.nodes.li)) {
    bind('Enter', splitListItem(type));
    bind('Mod-[', liftListItem(type));
    bind('Mod-]', sinkListItem(type));
  }
  if ((type = schema.nodes.p)) bind('Shift-Ctrl-0', setBlockType(type));
